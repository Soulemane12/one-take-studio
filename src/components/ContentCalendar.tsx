...
const ContentCalendar = () => {
  ...
  const filterContent = (type: string): ContentPiece[] => {
    if (type === "all") return generatedContent;
    return generatedContent.filter(c => c.type === type);
  };
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <Card className="mb-12 border-primary/30 bg-gradient-to-br from-primary/5 to-card">
          <CardHeader>
            ...
            <CardTitle className="text-2xl">{mockRant.title}</CardTitle>
            <div className="flex items-center gap-2 text-muted-foreground">
              <Clock className="w-4 h-4" />
              <span>{mockRant.duration}</span>
            </div>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2 mb-6">
              {mockRant.themes.map((theme) => (
            ...
            <div className="space-y-3">
              ...
              {mockRant.spicyMoments.map((moment, i) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-12">
          <Card className="text-center p-4">
            <div className="text-3xl font-bold text-foreground">{contentStats.totalPieces}</div>
            <div className="text-sm text-muted-foreground">Total Pieces</div>
          </Card>
          <Card className="text-center p-4">
            <div className="text-3xl font-bold text-[hsl(349,100%,60%)]">{contentStats.tiktok}</div>
            <div className="text-sm text-muted-foreground">TikTok Scripts</div>
          </Card>
          <Card className="text-center p-4">
            <div className="text-3xl font-bold text-[hsl(203,89%,53%)]">{contentStats.twitter}</div>
            <div className="text-sm text-muted-foreground">Twitter Threads</div>
          </Card>
          <Card className="text-center p-4">
            <div className="text-3xl font-bold text-[hsl(201,100%,35%)]">{contentStats.linkedin}</div>
            <div className="text-sm text-muted-foreground">LinkedIn Posts</div>
          </Card>
          <Card className="text-center p-4">
            <div className="text-3xl font-bold text-[hsl(142,71%,45%)]">{contentStats.newsletter}</div>
